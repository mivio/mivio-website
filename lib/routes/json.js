...
function sendVideos(req, res, next) {
	...
	if (req.query['startKey.id']) {
		req.query.startKey = {
			id: req.query['startKey.id'],
			createdAt: req.query['startKey.createdAt']
		};
	}
	...
	if (req.query.startKey) {
		...
		options.startKey.country = country;
	}

	if (category) {
		method = 'getLatestCategoryVideos';
		options.key = [country, category].join('-');
		options.startKey.countryCategory = options.key;
		delete options.startKey.country;
	}
