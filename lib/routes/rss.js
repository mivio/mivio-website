...
function sendVideos(req, res, next) {
	...
	if (category) {
		...
		options.key = [country, category.id].join('-');
	}
	...
	Data.access[method](options)
		.then(function(videos) {
			...
			var rssOptions = {
				...
				site_url: host,
				feed_url: host + links.rss.videos(),
				language: lang
			};

			if (category) {
				rssOptions.title = util.format(__('page_title_by_category'), category.name);
				rssOptions.feed_url = host + links.rss.videosByCategory(category.id);
			}
